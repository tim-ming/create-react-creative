...
const PM: ReadonlyArray<Readonly<PackageManagerConfig>> = [
  {
    name: "npm",
    install: ["install"],
...
function detectDefaultPM(): PackageManagerConfig {
  const userAgent = process.env.npm_config_user_agent || "";
  const pm =
    PM.find((pm) => userAgent.startsWith(pm.name)) ||
...
async function promptState(initial: WizardState): Promise<WizardState> {
    ...
    [
      ...
      },
      {
        ...
        initial: Math.max(
          0,
          PM.indexOf(
            (initial.packageManager as PackageManagerConfig) ||
      ...
      },
      {
        ...
        initial: ANIMATIONS.indexOf(
          (initial.animation as AnimationPackage) || DEFAULTS.animation
        ),
        hint: selectHint,
      },
      {
        ...
        initial: STATE_MANAGEMENTS.indexOf(
          (initial.stateManagement as StateManagementPackage) ||
      ...
      },
      {
        ...
        initial: THREES.indexOf(
          (initial.three as ThreePackage) || DEFAULTS.three
        ),
        hint: selectHint,
      },
      {
        type: (prev) =>
          prev == THREES.find((v) => v.packageName == "react-three-fiber")
            ? "multiselect"
            : null,
        name: "react-three",
  ...
  return {
    ...DEFAULTS,
    ...initial,
    ...(response as Partial<WizardState>),
  } as WizardState;
}
...
const summarize = (state: WizardState): string =>
  [
    ...
    `${kleur.bold().cyan("3D/Graphics")}: ${state.three} ${
      state.reactThree?.length ? state.reactThree.join(", ") : "None"
    }`,
...
async function ensurePmAvailable(
  ...
  if (!available) {
    ...
    const npm = PM.find((pm) => pm.name === "npm") as PackageManagerConfig;
...
function runDevCmd(pm: PackageManagerConfig): string {
  return pm.runDev;
}
