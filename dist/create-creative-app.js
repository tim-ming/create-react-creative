...
import { copyDirSafe, detectDefaultPM, ensurePmAvailable, extractExportName, flattenObjectValues, getAllRelativeFilePaths, parseFlags, } from "./helpers.js";
...
async function promptState(initial) {
    ...
    const { projectName, packageManager, animation, stateManagement, three, reactThree, creative, } = await p.group({
...
async function setupAppTsx(projectRoot, state) {
    ...
    for (const pkg of flattenObjectValues(state.scaffold)) {
        ...
        const exportName = extractExportName(await fs.readFile(path.join(SCAFFOLD_ROOT, pkg.demo.source, tsxFiles[0]), "utf8"));
        if (!exportName) {
...
const runSetup = async (projectRoot, state, noInstall) => {
    const steps = [
        {
...
async function installDeps(projectRoot, state, noInstall) {
    const devDeps = [
        "tailwindcss",
        "@tailwindcss/vite",
        "vite-tsconfig-paths",
    ...
    const install = async (packages, isDev) => packages.length > 0 &&
        execa(state.packageManager.name, state.packageManager.commands.addPkgs(packages, isDev), {
            cwd: projectRoot,
            stdio: "inherit",
        });
    if (noInstall)
