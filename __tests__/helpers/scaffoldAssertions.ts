...
function expectImport(appTsx: string, component: string) {
  expect(appTsx).toMatch(new RegExp(`import ${component} from ["']@\\/${component}["];?`));
}

export function assertRecTemplate(cwd: string, appName: string) {
  ...
  const demoFiles = ['Title.tsx', 'Counter.tsx', 'BackgroundScene.tsx', 'Lenis.tsx'];
  ...
  // Imports
  expectImport(appTsx, 'Title');
  expectImport(appTsx, 'Counter');
  expectImport(appTsx, 'BackgroundScene');
  expectImport(appTsx, 'Lenis');

  // JSX insertions
  expect(appTsx).toContain('<Title></Title>');
  expect(appTsx).toContain('<Counter></Counter>');
  expect(appTsx).toContain('<BackgroundScene></BackgroundScene>');
  expect(appTsx).toContain('<Lenis></Lenis>');
}

export function assertBarebonesTemplate(cwd: string, appName: string) {
  const appRoot = path.join(cwd, appName);
  ...
  // Demo files from selected libraries should be copied into src
  const demoFiles = ['Title.tsx', 'Counter.tsx'];
  for (const f of demoFiles) {
    expect(fs.pathExistsSync(path.join(appRoot, 'src', f))).toBe(true);
  }
